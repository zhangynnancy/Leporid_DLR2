...
def train(all_data, test_model, save_performance, draw_loss, add_sch):
    ...
    hyperparameters = {'dim': args.dim, 'norm': args.norm, 'lr': args.lr, 'spectopk': args.spec_topk,
                       'dis': args.spec_distance}
    ...
    for user in tqdm(user_set):
        for num in range(len(interact.train_file[user]) - args.state_num):
            cur_state, next_rec = interact.init(user, num)
            interact.sample.append([user, cur_state, next_rec, neg[user][:stored_neg]])
            random.shuffle(neg[user])

    # load some pre-trained samples
    interact.multi_store(stored_neg)
    ...
    if add_sch:
        f_sch = lr_scheduler.CosineAnnealingLR(interact.Feature_Gen_optim, T_max=args.max_iteration)
        c_sch = lr_scheduler.CosineAnnealingLR(interact.Critic_Network_optim, T_max=args.max_iteration)
        a_sch = lr_scheduler.CosineAnnealingLR(interact.Actor_Network_optim, T_max=args.max_iteration)

    for iteration in tqdm(range(args.max_iteration)):
        if (iteration + 1) % store_interval == 0:
            interact.multi_store(stored_neg)

        c_critic, c_actor = interact.update()
        ...
        if test_model and (iteration + 1) % 100 == 0:
            ...
            if save_performance:
                ...
                wb.save(
                    result_path + args.dataset + '_' + str(args.dim) + args.initialization + str(args.lr) + '_' + str(
                        args.spec_topk) + str(args.spec_distance) + str(args.coefficient) + '.xls')
            ...
            if best_test_actor < cur_best_test_actor:
                ...
                d = args.initial_type + str(args.dim) + '_bestActor/'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--initial_type', default='leporid', help='degree_centrality, le, strong_reg, leporid')
    ...
    parser.add_argument('--max_iteration', default=5000, help='max step for pre train process')
